...
export class AppointmentsService {
    ...
    async createAppointment(caller: CallerContext, dto: CreateAppointmentDto): Promise<Appointment> {
        ...
        if (!APPOINTMENT_RECIPIENT_ROLES.includes(convocadoA.rol as AppointmentRecipientRole)) {
        ...
        if (convocadoA.rol === 'psicologa') {
            await this.assertSlotFitsAvailability(convocadoA.id, scheduledAt, durationMin);
        }
        ...
        return this.dataSource.transaction('SERIALIZABLE', async (em) => {
            ...
            // Auto-asignar alumno↔psicóloga si la contraparte es psicóloga
            if (convocadoA.rol === 'psicologa') {
                await this.upsertAssignment(em, convocadoA.id, dto.studentId);
            }
            // O si el creador es psicóloga (caso: psicóloga cita al padre)
    ...
    async listMine(caller: CallerContext, q: ListAppointmentsQueryDto) {
        ...
        const qb = this.baseAppointmentQuery()
            .where(new Brackets((qb2) => {
                qb2.where('a.convocado_por_id = :id', { id: caller.id })
    ...
    async listByStudent(caller: CallerContext, studentId: string, q: ListAppointmentsQueryDto) {
        ...
        const qb = this.baseAppointmentQuery()
            .where('a.alumno_id = :studentId', { studentId });
        if (q.estado) qb.andWhere('a.estado = :estado', { estado: q.estado });
        if (q.from) qb.andWhere('a.fecha_hora >= :from', { from: new Date(q.from) });
        if (q.to) qb.andWhere('a.fecha_hora <= :to', { to: new Date(q.to) });
    ...
    async updateAppointment(
        ...
        if (dto.scheduledAt) {
            ...
            const recipient = await this.cuentaRepo.findOne({
                where: { id: appt.convocadoAId }, select: ['id', 'rol'],
            });
    ...
    private assertScheduledAtIsValid(d: Date): void {
        ...
        if (d > max) {
            throw new BadRequestException(`No se puede agendar a más de ${MAX_FUTURE_MONTHS} meses`);
        }
    }
    ...
    private assertStateTransition(
        ...
        if (next === 'realizada' || next === 'no_asistio') {
            if (caller.rol !== 'admin' && appt.convocadoAId !== caller.id) {
                throw new ForbiddenException('Solo el convocado puede marcar la cita como realizada/no asistió');
            }
        }
    }
    ...
    private async assertCanInvolveStudent(caller: CallerContext, studentId: string): Promise<void> {
        ...
        switch (caller.rol) {
            ...
            case 'padre': {
                ...
                if (linked.length === 0) {
                    throw new ForbiddenException('Ese alumno no está vinculado a tu cuenta');
                }
                return;
            }
            ...
            case 'alumno':
                if (caller.id !== studentId) {
                    throw new ForbiddenException('Un alumno solo puede agendar citas sobre sí mismo');
                }
                return;
    ...
    private async assertParentBelongsToStudent(parentId: string, studentId: string): Promise<void> {
        ...
        if (linked.length === 0) {
            throw new BadRequestException('Ese padre no corresponde al alumno indicado');
        }
    }
    ...
    private async assertSlotFitsAvailability(
        ...
        const blocked = await this.blockRepo
            ...
            .andWhere('b.fecha_inicio <= :end AND b.fecha_fin >= :start', { start, end })
            .getCount();
        if (blocked > 0) {
            throw new BadRequestException('La psicóloga tiene un bloqueo en ese rango');
        }
        ...
        if (ignoreAppointmentId) {
            overlapQB.andWhere('a.id <> :ignoreId', { ignoreId: ignoreAppointmentId });
        }
        const overlap = await overlapQB.getCount();
        if (overlap > 0) throw new ConflictException('Ese horario ya está ocupado');
    }

    private async upsertAssignment(em: any, psychologistId: string, studentId: string): Promise<void> {
        await em.query(
